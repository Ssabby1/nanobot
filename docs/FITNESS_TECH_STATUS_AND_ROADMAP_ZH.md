...
本次更新完成了健身模块从“规则路由完成版”向“LLM 路由接入版”的推进。需要特别说明的是：本文档原有大量内容仍然以“LLM 路由尚未接入”为背景撰写，但截至 2026-03-29，实际代码状态已经更新为：

- 健身业务基础版已完成
- 规则路由版已完成
- LLM 路由已接入，并作为规则路由未命中时的结构化兜底层

当前实际架构已经变为：

1. 规则优先
2. LLM 兜底
3. 统一复用 `FitnessService` 执行层

### 0.1 本次已完成的核心实现

- 新增 `nanobot/fitness/llm_router.py`
- 新增 `FitnessLLMRouter`
- LLM 路由通过 provider 的 tool/function call 返回结构化结果，而不是自由文本
- 结构化返回字段包括：
  - `is_fitness_request`
  - `action`
  - `confidence`
  ...
  - `needs_followup`
  - `assistant_reply`
- `fitness route` 已接入 LLM 兜底
- `nanobot agent -m "..."` 的 fitness 短路入口已接入 LLM 兜底

### 0.2 当前 LLM 路由的工作方式

当前不是让 LLM 直接调用业务逻辑，也不是让 LLM 直接输出自由答案，而是采用以下约束链路：

1. 用户输入先经过规则路由判断
2. 如果规则命中且信息充分，则直接执行
3. 如果规则未命中或缺参较多，则进入 `FitnessLLMRouter`
4. LLM 只负责输出结构化 action 和 arguments
5. 最终仍由现有 `FitnessService` 执行建档、更新、计划、打卡、建议等动作

这意味着当前版本的 LLM 路由层定位是“结构化理解层”，不是“自由执行层”。

### 0.3 当前补槽交互也已升级

在接入 LLM 路由的同时，当前补槽体验也已经从“只提示缺参”升级为“可继续完成”的状态：

- 缺参时会返回必填字段提示模板
- 未完成动作会记录到 `workspace/fitness/router_state.json`
- 用户下一句只补充缺失信息即可继续完成
- 这套 pending state 现在同时可被规则路由和 LLM 路由复用

### 0.4 当前已验证的新增链路

除原有规则路由链路外，本次还补充并手动验证了以下新增能力：

- `agent -m` 在规则未命中时可回退到 LLM 路由
- LLM 路由可将自然语言更新请求转换为结构化 `update_profile`
- LLM 路由可在 pending profile 状态下继续补全建档
- LLM 路由完成后仍然输出统一的业务格式化结果，而不是杂乱自由文本

### 0.6 第二版补充更新

在接入第一版 LLM 路由之后，当前已经继续完成第二版增强，主要包括：

- 低置信度不直接执行，而是优先返回澄清式追问
- “识别属于 fitness 域”与“足够自信可以执行”两件事已经拆开处理
- pending state 已从建档/更新扩展到打卡补槽
- 打卡缺参后，用户下一句只补“完成度 / 疲劳 / 饮食”也可以继续完成
- pending state 的落盘已补充 JSON-safe 序列化，避免 `date` 等字段导致状态写入失败

### 0.7 第三版补充更新

当前已经继续完成第三版增强，重点不再是“能否补槽”，而是“补槽追问是否自然”：

- 缺参提示不再默认一次性抛出整串字段
- 系统会优先追问最关键的 1 到 2 个字段
- 建档场景会优先追问：体重、训练经验
- 打卡场景会优先追问：完成度、疲劳程度
- 其余缺失字段会保留在 pending state 中，允许用户后续继续补充

因此，当前交互已经从“字段清单式报错”进一步升级为“带优先级的自然补槽追问”。

### 0.5 对本文档后续章节的阅读说明

下文第 1-10 节保留了规则路由阶段的大部分技术说明，仍然具有参考价值；但其中凡是出现“LLM 路由尚未接入”“下一步进入 LLM 路由阶段”等表述，应以上述 0 章的最新状态为准。
...
这次把多轮补槽里的追问文案升级成了“自然问题模板”，重点不再是告诉用户“缺哪些字段”，而是像真人一样优先问最关键的 1 到 2 个问题。

- 规则路由和 LLM 路由现在统一复用同一套追问生成逻辑，避免一边像真人、一边像字段表。
- 建档场景会输出更自然的组合问题，例如“你现在体重大概多少？另外你这次更偏向减脂还是增肌？”
- 打卡场景会根据上下文切换问题模板，休息日优先问“今天整体状态感觉累不累？饮食大概达标了吗？”
- 追问里依然保留直接可复制的回答示例，方便用户一条消息补齐最关键的信息。

## 0.9 第六版补充更新

这次继续把追问体验往前推了一步，不只是“问得自然”，而是“会看上下文换语气”。

- 如果用户已经说出了部分基础信息，建档追问会先承接上下文，例如“基础信息我先记下了，再补两个关键的就能继续”。
- 休息日打卡会先承认这是休息日场景，再追问状态和饮食，不再像普通训练日那样生硬。
- LLM 路由现在直接复用规则路由的追问文案生成逻辑，缺参补槽时不会和规则层出现两套口吻。
- 低置信度澄清也改成更拟人化的说法，例如“我猜你的意思是想改一下现在的档案，但把握还不够高”。 

## 0.10 第七版补充更新

这次把多轮补槽的“连续感”补上了。系统现在会记住当前已经追问到第几轮，第二轮、第三轮不会再重复第一次的开场语。

- pending state 里新增了追问轮次信息，续填时会自动切换成“我们接着把剩下的补完”这类承接式说法。
- 规则路由和 LLM 路由都会带上这层追问轮次，所以两条链路的多轮体验保持一致。
- 建档、更新、打卡都已经接入这套连续追问逻辑，尤其适合用户每次只补一句的真实使用方式。
